...
module.exports = function elasticSearchPlugin(schema, options) {
    ...
    schema.methods.index = function (index, type, cb, options) {
        ...
        esClient.index(index || indexName, type || typeName, serialize(model, mapping), options, cb);
    };
    ...
    function saveAndIndex(model, refresh, cb) {
        model.save(function (err) {
            if (err) return cb(err);
            model.index(cb, {refresh: refresh});
        });
    }

    schema.methods.saveAndIndexAsync = function (cb) {
        saveAndIndex(this, false, cb);
    };

    schema.methods.saveAndIndexSync = function (cb) {
        saveAndIndex(this, true, cb);
    };
    /**
    ...
    schema.statics.clearAll = function (cb) {
        var model = this;
        setIndexNameIfUnset(model.modelName);
        model.search({query: {match_all: {}}}, function (err, res) {
            if (res.hits.total > 0) {
                ...
                esClient.bulk(bulkData, {refresh: true}, function (err, res) {
                    cb(err, res);
                });
            } else cb(err, res);
        });
